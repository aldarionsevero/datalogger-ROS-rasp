...
def mode(pin, mode, force=False):
    assert 0 <= pin <= 27
    # Raspberry Pi rev 2
    legalPins = [2, 3, 4, 7, 8, 9, 10, 11, 14, 15, 17, 18, 22, 23, 24, 25, 27]
    # check pin or use dangerous: mode(88, "in", force=True)
    assert force or (pin in legalPins)
...
def pulseInHigh(pin):
    ...
    with open("/sys/class/gpio/gpio%i/value" % pin, 'r') as f:
        po = select.poll()
        # setup priority polling for more accurate time reading
...
def export(pin):
    ...
    try:
        ...
        if e.errno in (2, 13):  # 13-permission-denied, 2-no-such-file
            ...
            raise
    # wait for udev rule to fix permissions, seems to work with 0.1 s
    time.sleep(0.5)
